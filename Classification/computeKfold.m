function computeKfold(gen, t, numberOfSubsets)
    ...
    display(['(Positive group is "0") Responsivity = ' num2str(responsivity) 10]);

    specifity = numberOfNegativeMatches/sum(t==1);
    display(['(Negative group is "1") Specifity = ' num2str(specifity) 10 10]);
    showData(gen, t, 'Original data')
    showData(gen, results, ['Learnt data, number of subsets: ' num2str(numberOfSubsets)]);
end