function Solutions = HeuristicModified(Init, Final)
...
try
    ...
    cost(2) = cost(2) + 0.5 * sum(abs(vertcat(Blocks.ontable) - vertcat(Final.ontable)));
    cost(2) = cost(2) + 1.3 * sum(abs(vertcat(Blocks.clear) - vertcat(Final.clear)));
catch
    Possible=0;
end
